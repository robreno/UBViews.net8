...
{
    // TODO: paths below are wrong and need updated to Raw/Template folder
    // TODO: get paragraph methods don't get notes
    // TODO: Is FileService best place for Pararaph methods?
    ...
    private const string _query = "Templates";
    private const string _settings = "Templates";
    ...
    public async Task<string>LoadAsset(string fileName)
    {
        try 
        {
    ...
    public async Task<List<T>> LoadAsset_Obsolete<T>(string fileName)
    {
        try
        {
            string dataSource = Path.Combine("UBViews/", fileName);
    ...
    public async Task<List<Paragraph>> GetParagraphsAsync(int paperId)
    {
        try
        {
            ...
            var xdoc = XDocument.Parse(xml);
            var root = xdoc.Root;
            // Note: UbmlMauiBase is version used
