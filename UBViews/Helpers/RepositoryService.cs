...
namespace UBViews.Helpers
{
    public class RepositoryService : IRepositoryService
    {
        ...
        //private string postingDB_PathL = _postingDbPathLocalState;
        ...
        public async Task InititializeDatabase(IRepositoryService.InitOptions initOptions)
        {
            try
            {
                switch (initOptions)
                {
                    case IRepositoryService.InitOptions.QueryDB:
                        //QueryEngine.QueryRepository.connect(_queriesDbPathLocalState);
                        break;
                    case IRepositoryService.InitOptions.PostingDB:
        ...
        public async Task<(bool, QueryResultDto)> QueryResultExistsAsync(string queryString)
        {
            ...
            try
            {
                var result = await QueryEngine.QueryRepository.getQueryResultByStringAsync(queryDB_Path, queryString);
                var isSome = OptionModule.IsSome(result);
                if (isSome)
                {
                    var qr = result.Value;
                    var obj = new QueryResultDto
                    {
                        Id = qr.Id,
                        Hits = qr.Hits,
                        Type = qr.Type,
        ...
        public async Task<QueryResultLocationsDto> GetQueryResultByStringAsync(string queryString)
        {
            ...
            try
            {
                ...
                var isSome = OptionModule.IsSome(queryResult);
                if (isSome)
                {
        ...
        public async Task<QueryResultLocationsDto> GetQueryResultByIdAsync(int id)
        {
            ...
            {
                QueryResultLocationsDto queryResultLocationsDto = null;
                var queryResult = await QueryRepository.getQueryResultByIdAsync(queryDB_Path, id);
                var isSome = OptionModule.IsSome(queryResult);
                if (isSome)
                {
                    queryResultLocationsDto = await MapQueryResultObjToDto(queryResult.Value);
        ...
        public async Task<List<QueryCommandDto>> GetQueryCommandsAsync()
        {
            try
            {
                ...
                var queryResults = await QueryRepository.getQueryResultsAsync(queryDB_Path);
        ...
        public async Task<PostingListDto> GetPostingListByLexemeAsync(string lexeme)
        {
            ...
            try
            {
                ...
                var postingList = await PostingRepository.getPostingListByLexemeAsync(postingDB_Path, lexeme);
                var isSome = OptionModule.IsSome(postingList);
                if (isSome)
                {
                    postingListDto = await MapPostingListObjToDto(postingList.Value);
                }
        ...
        public async Task<List<TokenOccurrenceDto>> GetTokenOccurrencesByPostingListIdAsync(int postingId)
        {
            ...
            try
            {
                List<TokenOccurrenceDto> dtoList = null;
                var tokenOccs = await PostingRepository.getTokenOccurrencesByPostingListIdAsync(postingDB_Path, postingId);
                var isSome = OptionModule.IsSome(tokenOccs);
                if (isSome)
                {
                    dtoList = new List<TokenOccurrenceDto>();
        ...
        public async Task<List<TokenStemDto>> GetTokenStemsAsync()
        {
            ...
            try
            {
                List<TokenStemDto> dtoList = new List<TokenStemDto>();
                var tokenStems = await QueryEngine.PostingRepository.getTokenStemsAsync(postingDB_Path);
        ...
        public async Task<TokenStemDto> GetTokenStemAsync(string lexeme)
        {
            ...
            { 
                ...
                var isSom = OptionModule.IsSome(tokenStem);
                if (isSom)
                {
        ...
        public async Task<XElement> ProcessTokenPostingListAsync(string queryString, UBViews.Query.Ast.Query query, TokenPostingList tpl)
        {
            ...
            try
            {
                ...
                queryResult = QueryProcessor.processTokenPostingSequence(postingDB_Path, queryString, query, _tpl);
                return queryResult;
            }
            catch (Exception ex)
            {
