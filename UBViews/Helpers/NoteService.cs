...
public partial class NoteService : INoteService
{
    ...
    {
        string _method = "LastNoteIdAsync";
        try
        {
    ...
    public async Task<NoteEntry> CreateNoteEntryAsync(XElement note)
    {
        string _method = "CreateNoteEntryAsync";
        try
        {
            if (!_initialized)
            {
            ...
            var id = Int32.Parse(note.Attribute("id").Value);
            var type = note.Attribute("type").Value;
            var style = note.Attribute("style").Value;
            var paperId = Int32.Parse(note.Attribute("paperId").Value);
            var sequenceId = Int32.Parse(note.Attribute("seqId").Value);
            var locationId = note.Attribute("locationId").Value;
            var pid = note.Attribute("pid").Value;
    ...
    private async Task<List<NoteRun>> MakeRunsAsync(XElement runs)
    {
        ...
        try
        {
            ...
            {
                NoteRun newRun = new NoteRun();
                newRun.Text = _run.Value;

                var _attributes = _run.Attributes().ToList();  
                foreach (var att in _attributes)
                {
                    switch(att.Name.LocalName)
                    {
                        case "style":
                            newRun.Style = att.Value;
                            break;
                        case "textSize":
                            newRun.TextSize = att.Value;
                            break;
                        case "fontFamily":
                            newRun.FontFamily = att.Value;
                            break;
                        case "textColor":
                            newRun.TextColor = att.Value;
                            break;
                        case "textTransform":
                            newRun.TextTransform = att.Value;
                            break;
                        case "text":
                        default:
                            break;
    ...
    private async Task<DateTime> CreateDateTimeAsync(string date, string time)
    {
        ...
        try
        {
            ...
            var year = Int32.Parse(dateArr[0]);
            var month = Int32.Parse(dateArr[1]);
            var day = Int32.Parse(dateArr[2]);
    ...
    {
        string _method = "LoadAsset(fileName)";
        try
        {
    ...
    {
        string _method = "LoadAsset(rootPath, fileName)";
        try
        {
